...
export default function Admin() {
  ...
  useEffect(() => {
    axios.get(process.env.REACT_APP_FETCH)
      .then(res => {
        const users = res.data.users.filter(u => u._id !== localStorage.getItem("user"));
        if (users) {
          setUser(users);
        } else {
  ...
  return (
    <div className='admin'>
      ...
      <div className='shifts'>
        {shiftsFetch()}
        {dateList && selected ? <button onClick={handleSubmit} className='submitButton'>Zapisz zmiany</button> : ""}
      </div>
    </div>
  );
}
